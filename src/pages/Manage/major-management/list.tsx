...
const ListCategoryManagement = () => {
    ...
    const onDeleteCategory = async () => {
        setIsDeleteModal(false);
        try {
            await majorService.deleteMajors(
                String(idSelected),
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id: any) => {
        navigate(`${(ROUTE_PATH.VIEW_MAJOR_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
                ...
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon
                            icon={<PlusOutlined />}
                            classColor="green"
                            onClick={() => navigate(ROUTE_PATH.ADD_MAJOR_MANAGEMENT)}
                            title={"Thêm mới"} />
                    </Col>
                </Row>
            </div>
