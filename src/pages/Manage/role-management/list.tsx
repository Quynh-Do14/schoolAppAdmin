...
import roleService from '../../../infrastructure/repositories/role/role.service'
...
const ListRoleManagement = () => {
    ...
    const onNavigate = (id: any) => {
        navigate(`${(ROUTE_PATH.VIEW_ROLE_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <ManageLayout breadcrumb={"Quản lý quyền"} title={"Danh sách quyền"} redirect={""}>
                ...
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon
                            icon={<PlusOutlined />}
                            classColor="green"
                            onClick={() => navigate(ROUTE_PATH.ADD_ROLE_MANAGEMENT)}
                            title={"Thêm mới"} />
                    </Col>
                </Row>
            </div>
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa quyền này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa quyền"}
