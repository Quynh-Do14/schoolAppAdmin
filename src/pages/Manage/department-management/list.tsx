...
let timeout: any
const ListDepartmentManagement = () => {
    ...
    const onNavigate = (id: any) => {
        navigate(`${(ROUTE_PATH.VIEW_DEPARTMENT_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <ManageLayout breadcrumb={"Quản lý ngành"} title={"Danh sách ngành"} redirect={""}>
                ...
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    ...
                    <Col>
                        <ButtonCommon
                            icon={<PlusOutlined />}
                            classColor="green"
                            onClick={() => navigate(ROUTE_PATH.ADD_DEPARTMENT_MANAGEMENT)}
                            title={"Thêm mới"} />
                    </Col>
                </Row>
            </div>
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa ngành này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa ngành"}
...
export default ListDepartmentManagement