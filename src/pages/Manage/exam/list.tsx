...
import { convertHourShow } from '../../../infrastructure/helper/helper'

let timeout: any
const ListExamManagement = () => {
    ...
    const onNavigate = (id: any) => {
        navigate(`${(ROUTE_PATH.VIEW_EXAM_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
    return (
        <ManageLayout breadcrumb={"Quản lý lịch thi"} title={"Danh sách lịch thi"} redirect={""}>
                ...
                <Row className='filter-page mb-2 py-2-5' gutter={[10, 10]} justify={"space-between"} align={"middle"}>
                    <Col xs={24} sm={24} lg={16}>
                        <Row align={"middle"} gutter={[10, 10]}>
                            <Col xs={24} sm={12} lg={12}>
                                <InputSearchCommon
                                    placeholder="Tìm kiếm theo tên lịch thi..."
                                    value={searchText}
                    ...
                    <Col>
                        <ButtonCommon
                            icon={<PlusOutlined />}
                            classColor="green"
                            onClick={() => navigate(ROUTE_PATH.ADD_EXAM_MANAGEMENT)}
                            title={"Thêm mới"} />
                    </Col>
                </Row>
            </div>
            ...
            <DialogConfirmCommon
                message={"Bạn có muốn xóa lịch thi này ra khỏi hệ thống"}
                titleCancel={"Bỏ qua"}
                titleOk={"Xóa lịch thi"}
...
export default ListExamManagement